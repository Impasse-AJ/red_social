...
CREATE TABLE amistad (
    ...
    `usuarioA_id` INT NOT NULL,
    `usuarioB_id` INT NOT NULL,
    `aceptada` BOOLEAN DEFAULT FALSE,
    `fecha_solicitud` DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (`usuarioA_id`) REFERENCES `usuarios`(`id`) ON DELETE CASCADE,
    FOREIGN KEY (`usuarioB_id`) REFERENCES `usuarios`(`id`) ON DELETE CASCADE
);
